...
node {
    ...
    dir("${JENKINS_HOME}/workspace/${JOB_NAME}/${BUILD_NUMBER}") {
        ...
        env.PROJECT_NAME = 'polymesh-confidential-asset-ui'
            ...
            stage('Build') {
                sh (label: 'Build',
                    script: '''\
                            ...
                            docker build -f Dockerfile -t "${CONTAINER_REGISTRY}/polymesh/confidential-asset-ui:${GIT_COMMIT}" .
                            ''')
            }
            ...
            stage('Push') {
                sh (label: 'Push',
                    script: '''\
                            ...
                            docker push "${CONTAINER_REGISTRY}/polymesh/confidential-asset-ui:${GIT_COMMIT}" || true
                            ''')
            }

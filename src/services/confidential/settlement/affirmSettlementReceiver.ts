/**
 * Affirm Settlement as Receiver Service
 *
 * Handles receiver affirmation of settlement legs
 */
...
export interface AffirmSettlementReceiverParams {
  /**
...
export interface AffirmSettlementReceiverResult {
  /**
   * Transaction hash
   */
  txHash: string;
 ...
 */
export async function affirmSettlementReceiver(
  params: AffirmSettlementReceiverParams,
): Promise<AffirmSettlementReceiverResult> {
  const {
    settlementId,
    legId,
    assetId,
  ...
  const affirmationProof = accountAssetState.receiverAffirmProof(
    accountKeys,
    accountLeafPath,
    settlementId,
    legId,
  ...
  // Submit transaction
  onSubmitting?.();
  const tx = polkadotApi.tx.confidentialAssets.receiverAffirmation(proofBytes);
  const result = await submitTransaction({
    tx,
    tag: 'confidentialAssets.receiverAffirmation',
  });

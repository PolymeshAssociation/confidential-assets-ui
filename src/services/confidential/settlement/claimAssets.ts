/**
 * Claim Assets Service
 *
 * Handles receiver claiming assets from settlement legs
 */
...
export interface ClaimAssetsParams {
  /**
...
export interface ClaimAssetsResult {
  /**
   * Transaction hash
   */
  txHash: string;
...
/**
 * Claim assets from a settlement leg
 * This is the final step for the receiver to credit the transferred amount
 *
 * @param params - Claim parameters
 * @returns Transaction result with updated state
 */
export async function claimAssets(
  params: ClaimAssetsParams,
): Promise<ClaimAssetsResult> {
  const {
    settlementId,
    legId,
    assetId,
  ...
  const claimProof = accountAssetState.receiverClaimProof(
    accountKeys,
    accountLeafPath,
    settlementId,
    legId,
  ...
  const proofBytes = claimProof.toBytes();

  // Submit transaction
  onSubmitting?.();
  const tx = polkadotApi.tx.confidentialAssets.receiverClaim(proofBytes);
  const result = await submitTransaction({
    tx,
    tag: 'confidentialAssets.receiverClaim',
  });

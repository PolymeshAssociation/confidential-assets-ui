/**
 * Update Sender Counter Service
 *
 * Handles sender counter update after settlement execution
 */
...
export interface UpdateSenderCounterParams {
  /**
...
export interface UpdateSenderCounterResult {
  /**
   * Transaction hash
   */
  txHash: string;
 ...
 */
export async function updateSenderCounter(
  params: UpdateSenderCounterParams,
): Promise<UpdateSenderCounterResult> {
  const {
    settlementId,
    legId,
    assetId,
  ...
  const updateProof = accountAssetState.senderCounterUpdateProof(
    accountKeys,
    accountLeafPath,
    settlementId,
    legId,
  ...
  const proofBytes = updateProof.toBytes();

  // Submit transaction
  onSubmitting?.();
  const tx = polkadotApi.tx.confidentialAssets.senderUpdateCounter(proofBytes);
  const result = await submitTransaction({
    tx,
    tag: 'confidentialAssets.senderUpdateCounter',
  });
